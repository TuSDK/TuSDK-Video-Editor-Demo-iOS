...
{
    TuTextItemView *view = [[TuTextItemView alloc]init];
    [view setupUI:rect];
    return view;
}
...
- (void)handleTapGesture:(UITapGestureRecognizer *)recognizer {
    ...
    if (doubleClick == 2) {
        if ([self.delegate respondsToSelector:@selector(onDoubleClick:)]) {
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)recognizer;
{
    ...
    {
        //NSLog(@"hecc--Pan Began: vid= %d",_vid);
        if (!_dragBtn.hidden) {
    ...
    else if (recognizer.state == UIGestureRecognizerStateChanged)
    {
        if (_isRotatScaleAction) {
            ...
            //NSLog(@"hecc--Pan Changed: vid= %d",_vid);
...
- (void)handleRotationGesture:(UIRotationGestureRecognizer *)recognizer;
{
    ...
    if (recognizer.state == UIGestureRecognizerStateChanged)
    {
        ...
        info.type = TuTextItemView_TransformType_Rotate;
...
- (void)handlePanGestureTransAction:(CGPoint)nowPoint;
{
    ...
    // update builder
    TuTextItemInfo *info = [[TuTextItemInfo alloc]init];
    info.type = TuTextItemView_TransformType_Translate;
...
- (void)computerScaleWithScale:(CGFloat)scale center:(CGPoint)cPoint;
{
    ...
    // update builder
        TuTextItemInfo *info = [[TuTextItemInfo alloc]init];
    info.type = TuTextItemView_TransformType_Scale;
    info.scale = _mScale;
    if ([self.delegate respondsToSelector:@selector(updatePropBuilder:info:)]) {
        [self.delegate updatePropBuilder:_vid info:info];
    }
...
- (void)setSelect:(BOOL)select{
    //NSLog(@"hecc--select vid= %d, select= %d, touched= %d",_vid,select,_hasTouched);
...
-(void) redraw:(CGRect)rect rotation:(int)rotation {
    ...
    //NSLog(@"hecc--redraw vid= %d, [%f,%f]",_vid,self.center.x,self.center.y);
